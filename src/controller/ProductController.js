...
const registerProduct = async(req, res) => {
  ...
  try {
    ...
    if(existProduct){
      res.status(422).json("Opss!! Produto já cadastrado.")
      return
    }
...
const editProduct = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({success: true, msg: "Produto atualizado com sucesso!"})
...
const lastSales = async (req, res) => {

  try {

    const allProduct = await ProductModel.find().sort({updatedAt: -1, sales: -1}).limit(10)
